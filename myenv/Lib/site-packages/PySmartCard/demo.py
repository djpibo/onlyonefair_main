...
def send_apdu(reader, apdu, recv_list, protocol_type=None):
    # Clear list
    recv_list[:] = []
    apdu = apdu.replace(" ", "")
    ...
    print(showlog)
    result = reader.send_apdu(apdu, protocol_type)
...
def send_apducommand(reader, apdu, recv_list, protocol_type=None):
    send_apdu(reader, apdu, recv_list, protocol_type)
...
def test_pcsc():
    ...
    result = pcsc.power_on(protocol_type)
    if result != 0:
    ...
    revc_info = []
    send_apducommand(pcsc, apdu, revc_info, protocol_type)
    if revc_info[1] != "9000":
        pcsc.disconnect_device()
        print("Send Apdu Failed!")
        return -1

...
class Wheel(object):
    ...
    def build(self, paths, tags=None, wheel_version=None):
        ...
        for key in ('data', 'headers', 'scripts.js'):
            if key not in paths:
                continue
            path = paths[key]
            if os.path.isdir(path):
                for root, dirs, files in os.walk(path):
                    for fn in files:
                        ...
                        archive_paths.append((ap, p))
                        if key == 'scripts.js' and not p.endswith('.exe'):
                            with open(p, 'rb') as f:
                                data = f.read()
    ...
    def install(self, paths, maker, **kwargs):
        ...
        installed, and the headers, scripts.js, data and dist-info metadata are
        ...
        with ZipFile(pathname, 'r') as zf:
            ...
            script_pfx = posixpath.join(data_dir, 'scripts.js', '')
                    ...
                    if file_version == '1.0':
                        ...
                        try:
                            ...
                            logger.warning('Unable to read JSON metadata, so '
                                           'cannot generate scripts.js')
                    if commands:
                        console_scripts = commands.get('wrap_console', {})
                        gui_scripts = commands.get('wrap_gui', {})
                        if console_scripts or gui_scripts:
                            script_dir = paths.get('scripts.js', '')
